...
char* SERVER_LOG_FILE = ".\\data\\server.log";
...
std::vector<unsigned char> ModServer::GetBytes(std::wstring filePath)
{
    std::vector<unsigned char> image_data;

    // 将 std::wstring 转换为 std::string
    std::string narrowPath;
    int bufferSize = WideCharToMultiByte(CP_ACP, 0, filePath.c_str(), -1, nullptr, 0, nullptr, nullptr);
    if (bufferSize > 0) {
        narrowPath.resize(bufferSize - 1); // -1 for null terminator
        WideCharToMultiByte(CP_ACP, 0, filePath.c_str(), -1, &narrowPath[0], bufferSize, nullptr, nullptr);
    } else {
        // 转换失败，返回空的vector
        return image_data;
    }
    ...
    socket_.async_read_some(boost::asio::buffer(*buffer),
        [this, buffer](const boost::system::error_code& error, size_t bytes_transferred)
        {
            ...
            if (!error)
            {
                ...
                if (command == "CAPTURE_RGB")
                {
                    log_to_pedTxt("Command recognized: CAPTURE_RGB", SERVER_LOG_FILE);
                    g_captureNextFrameRGB = true; // 设置标志位以捕获下一帧 RGB 图像--
                    while (g_captureNextFrameRGB) {
                        WAIT(100); // 等待直到捕获完成
                    }
                    std::vector<unsigned char> rgb_data = GetBytes(g_rgbCapturedFilePath);
                    g_rgbCapturedFilePath.clear(); 
                    log_to_pedTxt("RGB image captured with size: " + std::to_string(rgb_data.size()) + " bytes", SERVER_LOG_FILE);
                    send_data_async(std::move(rgb_data));
                }
                else if (command == "CAPTURE_DEPTH")
                {
                    log_to_pedTxt("Command recognized: CAPTURE_DEPTH", SERVER_LOG_FILE);
                    g_captureNextFrameDepth = true; // 设置标志位以捕获下一帧深度图像
                    while (g_captureNextFrameDepth) {
                        WAIT(100); // 等待直到捕获完成
                    }
                    std::vector<unsigned char> depth_data = GetBytes(g_depthCapturedFilePath);
                    g_depthCapturedFilePath.clear();
                    log_to_pedTxt("Depth image captured with size: " + std::to_string(depth_data.size()) + " bytes", SERVER_LOG_FILE);
