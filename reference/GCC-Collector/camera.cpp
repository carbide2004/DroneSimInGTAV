...
void adjustCamera()
{
	...
	if (g_cmdQueue.size() > 0) {
		...
		g_cmdQueue.pop();
		log_to_file("front cmd is: " + cmd);
		if (cmd == "FORWARD") {
			camDelta.x = 10.0;
			isMovement = true;
			set_status_text("Camera moving forward.");
		}
		else if (cmd == "BACKWARD") {
			camDelta.x = -10.0;
			isMovement = true;
			set_status_text("Camera moving backward.");
		}
		else if (cmd == "LEFT") {
			camDelta.y = -10.0;
			isMovement = true;
			set_status_text("Camera moving left.");
		}
		else if (cmd == "RIGHT") {
			camDelta.y = 10.0;
			isMovement = true;
			set_status_text("Camera moving right.");
		}
		else if (cmd == "UP") {
			camDelta.z = 3;
			isMovement = true;
			set_status_text("Camera moving up.");
		}
		else if (cmd == "DOWN") {
			camDelta.z = -3;
			isMovement = true;
			set_status_text("Camera moving down.");
		}
			...
			set_status_text("Camera rotating left.");
		}
		else if (cmd == "RIGHTROTATE") {
			Vector3 currentRotation = CAM::GET_CAM_ROT(cameraHandle, 2);
			currentRotation.z += -15.0f;
			CAM::SET_CAM_ROT(cameraHandle, currentRotation.x, currentRotation.y, currentRotation.z, 2);
			set_status_text("Camera rotating right.");
		}
	}
	if (isMovement) {
		if (shift.isKeyDown()) {
