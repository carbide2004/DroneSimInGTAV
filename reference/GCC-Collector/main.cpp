...
void clear_depth_stencil_view_hook(ID3D11DeviceContext* self, ID3D11DepthStencilView* dsv, UINT8 flags, float depth, UINT8 stencil)
{
    ...
    if (curDSV != nullptr) {
        ...
        // 修改触发逻辑：当g_captureNextFrameDepth为true且DSV尺寸合适时进行捕获
        if (lastDsv == nullptr && desc.Format == DXGI_FORMAT_R32G8X24_TYPELESS && g_captureNextFrameDepth && desc.Width > 600 && desc.Height > 600) { 
            // 将g_captureNextFrameDepth重置为false，只捕获一帧
            lastDsv = curDSV;

            std::chrono::milliseconds ms = std::chrono::duration_cast< std::chrono::milliseconds >(
                std::chrono::system_clock::now().time_since_epoch()
            );
            FILE* f = fopen(logFilePath, "a"); // 继续使用logFilePath记录日志
            ...
            // 构造新的保存路径
            char currentRawPath[MAX_PATH]; // 假设MAX_PATH已定义，或使用std::string
            sprintf(currentRawPath, ".\\data\\temp_capture_depth_%lld.raw", ms.count()); // 使用sprintf格式化路径并递增计数器
			g_depthCapturedFilePath = std::wstring(currentRawPath, currentRawPath + strlen(currentRawPath)); // 更新全局变量以存储捕获的文件路径

            if (!onlyScreen) {
                screenShot(f); // 截图，日志记录到f
                auto raw = fopen(currentRawPath, "wb"); // 使用wb模式写入二进制文件
                if (raw) { // 检查文件是否成功打开
                    fwrite(buf, 1, size, raw);
                    fclose(raw);
                    fprintf(f, "[%I64d] : Wrote stencil to file: %s.\n", ms.count(), currentRawPath);
					log_to_file("Depth image saved successfully to "s + std::string(currentRawPath));
                } else {
                    fprintf(f, "[%I64d] : Failed to open file for writing: %s.\n", ms.count(), currentRawPath);
                }
            } else {
                onlyScreen = false;
                bool writeBmp = true;
			...
			g_captureNextFrameDepth = false; 
            // makeCmdStop(); // 根据新逻辑，此行可能不再需要，因为g_captureNextFrameDepth已重置
            fclose(f);
        }
    }
...
bool g_captureNextFrameRGB = false;
bool g_captureNextFrameDepth = false;
std::wstring g_rgbCapturedFilePath; 
std::wstring g_depthCapturedFilePath;
std::queue<std::string> g_cmdQueue; // 用于存储命令队列
...
{	
	...
	// 如果触发了截图
    if (g_captureNextFrameRGB)
	{
		// 使用 DirectX Tool Kit 保存图像
		// 确保你的项目包含了这个库并正确配置-
		std::chrono::milliseconds ms_now = std::chrono::duration_cast<std::chrono::milliseconds>(
          std::chrono::system_clock::now().time_since_epoch()
      	);
		std::wstring filePath = L".\\data\\temp_capture_rgb_" + std::to_wstring(ms_now.count()) + L".png";
		HRESULT hr = DirectX::SaveWICTextureToFile(
			ctx.Get(), 
			backBuffer.Get(), 
			GUID_ContainerFormatPng, // 保存为 PNG 格式
            filePath.c_str()
		);
	
		if (SUCCEEDED(hr)) {
            log_to_file("RGB image saved successfully.");
		} else {
			log_to_file("Failed to save RGB image.");
		}

		g_captureNextFrameRGB = false; // 重置标志位
		g_rgbCapturedFilePath = filePath; // 更新全局变量以存储捕获的文件路径
	}
