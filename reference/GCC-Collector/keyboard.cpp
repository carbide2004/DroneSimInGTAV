...
void OnKeyboardMessage(DWORD key, WORD repeats, BYTE scanCode, BOOL isExtended, BOOL isWithAlt, BOOL wasDownBefore, BOOL isUpNow)
{
	...
	if (scriptStatus == scriptStop && push(VK_F9)) {
		...
		set_status_text("Ready to start the camera mode, press F10 to start.");
	}
	if (scriptStatus == scriptReady && push(VK_F10)) {
	...
	if (scriptStatus == cameraMode && CameraMode == true) {
		...
		if (push(VK_F11)) scriptStatus = scriptStop;
		if (push(VK_F10)) {
			set_status_text("Capturing ...");
			g_captureNextFrameRGB = true;
            g_captureNextFrameDepth = true;
			// 防止连续触发
            WAIT(500); // 等待半秒
		}
